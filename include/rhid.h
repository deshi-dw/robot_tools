...
// TODO reduce the amount of static definitions in general.
...
typedef struct rhid_native_t rhid_native_t;
...
typedef struct {
	...
	// TODO move _preparsed to rhid_native_t
	void* _preparsed;
	rhid_native_t* native;
...
#define MAX_BUTTON_COUNT 32
	struct rhid_button_descriptor_t {
		uint8_t	 report_id;
		uint16_t page;
		uint16_t usage;
		int		 index;
	} button_descriptors[MAX_BUTTON_COUNT];

#define MAX_VALUE_COUNT 16
	struct rhid_value_descriptor_t {
		uint8_t	 report_id;
		int page;
		int usage;
		int		 logical_min;
		int		 logical_max;
		int		 index;
	} value_descriptors[MAX_VALUE_COUNT];
